...
from utils.vision import process_aruco, process_blob #new
...
class Drone:
    ...
    def connect(self, ip, port):
        ...
        self._image_processing_timer.start() #new
    ...
    def sim_kinematics_callback(self):
        ...
        self._sim_img = self._client.get_camera_capture() #new
        self._sim_ultrasonic = self._client.get_range_data(rangefinder_id = 0, range_min = 0.15, range_max = 4, is_clear = True, range_error = 0.0003)
    ...
    def image_processing_callback(self): 
        ...
        img_aruco = self._sim_img.copy()
        img_blob = self._sim_img.copy()
        self._aruco_data, self._camera_pose_aruco_data, self._aruco_img = process_aruco(img_aruco)

        if self._aruco_img is None:
            self._aruco_img = img_aruco

        self._blob_data, self._blob_img = process_blob(img_blob)
        if self._blob_img is None:
            self._blob_img = img_blob
